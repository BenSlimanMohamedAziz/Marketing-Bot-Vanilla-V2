...
async def generate_video_with_logo(prompt: str, logo_url: str) -> Optional[str]:
    ...
    for api_token in API_TOKENS:
        try:
            logger.info(f"Trying API token: {api_token[:10]}...")
            ...
            # Generate video using Replicate (run in thread pool as it's blocking)
            loop = asyncio.get_event_loop()
            ...
            logger.error(f"❌ Error with current API token: {e}")
